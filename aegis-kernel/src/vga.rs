...
//! Provides a basic 80x25 character display output for early boot messages.
...
impl Buffer {
    ...
    fn write(&mut self, row: usize, col: usize, val: ScreenChar) {
        unsafe { core::ptr::write_volatile(&mut self.chars[row][col], val) }
    }
}
    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                ...
                _ => self.write_byte(0xfe), // unprintable
            }
        }
    }
...
lazy_static! {
    /// Global VGA writer — AEGIS uses green-on-black for the security theme.
...
/// Print to VGA text buffer with newline.
#[macro_export]
macro_rules! println {
    () => ($crate::print!("\n"));
    ($($arg:tt)*) => ($crate::print!("{}\n", format_args!($($arg)*)));
